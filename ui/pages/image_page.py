        ...
        try:
            ...
            if _ref_script:
                with st.expander("📋 整形後の参考台本", expanded=False):
                    st.text_area("", value=_ref_script, height=120, disabled=True, key="image_ref_script", label_visibility="collapsed")
            _ref_core = (script_data.get("reference_script_core_normalized") or "").strip()
            if _ref_core:
                with st.expander("📋 整形後の参考台本核心部", expanded=False):
                    st.text_area("", value=_ref_core, height=80, disabled=True, key="image_ref_core", label_visibility="collapsed")
            _ref_meta = (script_data.get("reference_metadata") or "").strip()
            if _ref_meta:
                with st.expander("📋 人気動画のタイトル・概要（参考）", expanded=False):
                    st.text_area("", value=_ref_meta, height=100, disabled=True, key="image_ref_metadata", label_visibility="collapsed")
            # 人気動画を参考にしたタイトル・概要案
            ...
            if _st_ref or _sd_ref:
                with st.expander("📌 人気動画を参考にしたタイトル・概要案", expanded=False):
                    if _st_ref:
                        st.caption("**タイトル案**")
                        st.text_area("", value=_st_ref, height=40, disabled=True, key="image_title_ref", label_visibility="collapsed")
                    if _sd_ref:
                        st.caption("**概要案**")
                        st.text_area("", value=_sd_ref, height=80, disabled=True, key="image_desc_ref", label_visibility="collapsed")
