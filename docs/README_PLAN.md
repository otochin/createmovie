...
- **入力**: 
  ...
  - 参考台本核心部（オプション）をテキストエリアに入力（核心抽出の手がかりとして利用）
  - 人気動画のタイトル・概要（参考・オプション）をテキストエリアに入力
  - 台本生成指示（オプション）をテキストエリアに入力
- **処理**: 
  1. 参考台本が提供された場合、**前処理**としてトピックに合わせて「性教育動画の台本」として整える（誤字脱字・タイムスタンプ・[音楽]等の除去）。整えた参考台本・核心部をその後の処理に利用
  2. 整えた参考台本から、GPT-4oで視聴者のインサイト・知識・**核心部分**を抽出
  3. 抽出したインサイトを基に、そのインサイトを満足させる台本を生成
  4. 台本の後半で**核心部分**を反映するようプロンプトで指示（参考台本核心部があれば抽出の手がかりとして渡す）
  5. 台本生成指示が提供された場合、その指示を考慮して台本を生成
  6. **人気動画のタイトル・概要（参考）**がある場合のみ: そのテキスト**のみ**からタグを抽出（`suggested_tags`）。同じくそのテキストのみから「人気動画を参考にしたタイトル案・概要案」を生成（本編に触れず表現を少し変える）
  7. 参考台本がない場合は、トピックから直接台本を生成
- **出力形式**: JSON形式
  ```json
  {
    "title": "動画タイトル",
  ...
  ```
- **保存時に追加されるメタデータ**（台本JSONに含まれる）: `topic`, `reference_script_normalized`, `reference_script_core_normalized`, `reference_metadata`, `suggested_title_from_reference`, `suggested_description_from_reference`, `suggested_tags`（人気動画のタイトル・概要のみから抽出。参考が無い場合は空配列）
- **機能**:
  - 参考台本からの視聴者インサイト抽出
  - 参考台本からの知識抽出（具体的な事実、データ、専門知識、ノウハウなど）
  - 抽出したインサイトと知識の表示
  - インサイトと知識を基にした台本生成
  - 台本生成時にGPTが各シーンの`dialogue_for_tts`を返却（ひらがな・必要に応じカタカナ、句読点「、」「。」を**こまめに**挿入するようプロンプトで指示）。未返却時のみローカルでひらがな変換して補完
  - 参考台本から知識・**核心部分**も抽出する機能（インサイト・知識・核心の3つ）。核心は台本後半で反映
  - **サムネイル用テキスト案**: GPTで3案を生成し、台本ページに表示・コピー可能（Canva用など）
  ...
  - 再生成機能
  - 台本生成指示の初期値設定（デフォルト指示あり。**冒頭フック**「最初のシーンで視聴者を引き込むフックを入れる」、**核心部ネタバレ防止**「整えた参考台本核心部の内容は前半のセリフでは絶対に言わない。核心部分は必ず後半で説明する」を含む）
  - セリフの長さの調整（1秒あたり**12〜16文字**程度、詳細で具体的な内容）
  - デフォルト値: 総時間180秒、シーン数18、スタイル「教育」
  - **台本入力の永続化**: 別画面に移動して戻っても入力内容は復元しない（入力項目間のフォーカス移動で値が消える問題を避けるため）
  - **台本ファイルに保存するメタデータ**: トピック、整形後の参考台本、整形後の参考台本核心部、人気動画のタイトル・概要（参考）、人気動画を参考にしたタイトル案・概要案（`suggested_title_from_reference`, `suggested_description_from_reference`）。タグ（`suggested_tags`）は**人気動画のタイトル・概要（参考）のみ**から抽出し、参考が無い場合は空配列
  - **音声生成・画像生成・動画編集・台本生成画面**: 選択または表示中の台本について、上記メタデータ（トピック、整形後参考台本・核心部、人気動画タイトル概要、人気動画を参考にしたタイトル・概要案、タグ、概要説明）を表示可能（expander 等で折りたたみ）
...
- **機能**:
  ...
  - **ストック画像紐づけ機能**（API不使用）:
    ...
    - **今回の紐づけ実行内でのみ**同じ画像の重複を避ける。何度でも実行可能（前回実行との使用済み追跡は行わない）
    - 拡張子の自動正規化（小文字統一）
  - **画像を指定**（API不使用）:
    - 各シーンでPCから画像ファイルを選択して割り当て可能（file_uploader）
    - 選択した画像は動画サイズにリサイズして保存。**透過PNGは透過を保持**（背景動画が透けて見える）
    - マッピングに保存され、動画編集画面に反映される
  - **動画フォーマットの連動**:
    - 画像生成画面で選択した動画フォーマット（ショート/長尺）は、動画編集画面と共通の設定として連動する（セッション＋クッキーで保持）
  - **画像マッピング機能**:
    - 画像生成画面で割り当てた画像情報を**絶対パス**で保存（ショート: `{台本名}_image_mapping.json`、長尺: `{台本名}_image_mapping_long.json`）。読み込み時に `resolve` で正規化し動画編集への反映を安定化
    - 動画編集画面で同じ台本・同じフォーマットを選択すると、割り当てた画像が自動的に反映される
    - 画像生成画面では表示のたびにマッピングを読み込み、`generated_images` を同期して一覧に反映する
...
### 2.6 動画検索機能（予定）
- **目的**: ここ最近再生数が伸びているYouTube動画を検索し、サムネイルを参考にする。
- **配置**: 左ナビゲーションに「🔍 動画検索」を追加。
- **検索**: キーワード検索、公開時期の絞り込み（直近24時間／7日／30日／3ヶ月、または日付範囲）。
- **表示**: 検索結果のサムネイル・タイトル・チャンネル名・公開日・再生数、YouTubeへのリンク。
- **技術**: YouTube Data API v3（APIキーは環境変数で管理）。
- **詳細**: [docs/SPEC_VIDEO_SEARCH.md](SPEC_VIDEO_SEARCH.md) を参照。
...
│   ├── CHANGELOG.md           # 変更履歴
│   ├── SPEC_VIDEO_SEARCH.md   # 動画検索機能 仕様書
│   └── TODO.md                # TODOリスト
│
...
│   ├── youtube_client.py      # YouTube Data API v3 検索（動画検索機能・予定）
│   ├── logger.py              # ログ管理
│   └── validators.py          # 入力値検証
│
├── ui/                         # UIコンポーネント
...
│   │   └── video_search_page.py  # 動画検索画面（予定）
│   └── styles.py             # CSSスタイル定義
│
└── output/                     # 生成物の保存先
    ├── scripts/               # 生成された台本JSON
