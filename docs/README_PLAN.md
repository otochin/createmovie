    ...
    "scenes": [
      {
        "scene_number": 1,
        "dialogue": "セリフテキスト",
        "dialogue_for_tts": "せりふてきすと（ひらがな変換済み、カタカナはそのまま）",
...
- **機能**:
  ...
  - 抽出したインサイトと知識の表示
  - インサイトと知識を基にした台本生成
  - 各シーンのdialogueをひらがなに変換した`dialogue_for_tts`フィールドの自動生成（カタカナはそのまま保持）
...
- **機能**:
  ...
  - `dialogue_for_tts`はカタカナを保持（固有名詞や専門用語の読み上げが正確）
  - 音声のプレビュー再生
  - 音声の再生成機能
  - 音声ファイルの保存管理
...
- **機能**:
  ...
  - 画像ファイルの保存管理
  - **ストック画像紐づけ機能**（API不使用）:
    - `output/stock_images/`フォルダの画像をシーンにランダム割り当て
    - 同じ画像は一度のみ使用（重複なし、セッション内で追跡）
    - 拡張子の自動正規化（小文字統一）
      - **画像マッピング機能**:
      - 画像生成画面で割り当てた画像情報を保存
      - 動画編集画面で同じ台本を選択すると、割り当てた画像が自動的に反映される
      - マッピング情報は`output/scripts/{台本名}_image_mapping.json`に保存
      - 画像マッピングファイル（`*_image_mapping.json`）は台本一覧から除外される（台本として読み込まれない）
...
- **機能**:
  ...
  - **背景動画機能**:
    - `output/bgvideos/`フォルダの動画を背景として使用
    - 動画の長さに合わせて自動ループ再生
    - セレクトボックスで背景動画を選択
  - **BGM機能**:
...
└── output/                     # 生成物の保存先
    ...
    ├── images/                # 生成された画像ファイル
    ├── videos/                # 生成された動画ファイル
    ├── stock_images/          # ストック画像（ユーザーが配置）
    ├── bgvideos/              # 背景動画（ユーザーが配置）
    └── bgm/                   # BGMファイル（ユーザーが配置）
```
